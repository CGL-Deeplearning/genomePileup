...
for i in ${chrs[@]};
    ...
    echo "Starting chr" $i
    python3 main.py --bam $bam_file --ref $ref_file --vcf $vcf_file --vcf_region $i --output_dir $current_output_dir --max_threads $max_threads --parallel True 2>tmp/progress-$i.txt &
    done
